...
echo ""
echo "echo "%files""
echo "find $PACKAGE -type f -printf "/%P\n""
echo ""
echo "%post"
echo "#!/bin/bash -e"
echo "# Update library paths"
echo "sudo /sbin/ldconfig"
echo "# Update MIME type database"
echo "sudo /usr/bin/update-mime-database \"/usr/share/mime/\" 2> /dev/null"
echo "# Update desktop entry database"
echo "[ -x /usr/bin/update-desktop-database ] && sudo update-desktop-database -q 2> /dev/null"
echo "# Add bash completion script if bash-completion is installed"
echo "[ -d /etc/bash_completion.d ] && sudo ln -sf /usr/share/argos3/argos3_bash_completion.sh /etc/bash_completion.d/argos3"
echo ""
echo "%changelog"
